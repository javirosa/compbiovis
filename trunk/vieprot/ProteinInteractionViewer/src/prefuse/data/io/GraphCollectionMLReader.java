...
/**
 * Extends GraphMLReader
 */
public class GraphCollectionMLReader extends AbstractGraphReader  implements GraphReader {
    ...
    public Graph readGraph(InputStream is) throws DataIOException {
        try {       
            ...
            MultipleGraphMLHandler   handler   = new MultipleGraphMLHandler();
            saxParser.parse(is, handler);
            return handler.getGraphCollection().getGraph(0);
        } catch ( Exception e ) {
    ...
    public GraphCollection readGraphCollection(String location) throws DataIOException
    {
        try {
            ...
            return readGraphCollection(is);
        } catch ( IOException e ) {
            throw new DataIOException(e);
        }   
    }    
    
    public GraphCollection readGraphCollection(InputStream is) throws DataIOException {
        try {       
            SAXParserFactory factory   = SAXParserFactory.newInstance();
            SAXParser        saxParser = factory.newSAXParser();
            ...
            MultipleGraphMLHandler   handler   = new MultipleGraphMLHandler();
            saxParser.parse(is, handler);
            return handler.getGraphCollection();
        } catch ( Exception e ) {
            if ( e instanceof DataIOException ) {
                throw (DataIOException)e;
            } else {
     ...
     */
    public static class MultipleGraphMLHandler extends DefaultHandler implements Tokens
    {
        ...
        protected GraphCollection m_graphs = new GraphCollection();
...
//                m_edges.setBoolean(m_row, DIRECTED, d);
        ...
        public void addCurrentGraph() throws SAXException {
            ...
            Graph m_graph = new Graph(m_nodes, m_edges, m_directed);
            if (m_graphid != null)
                m_graph.putClientProperty(ID, m_graphid);
            
            m_graphs.addGraph(m_graph);
            resetCurrentGraph();
        }
